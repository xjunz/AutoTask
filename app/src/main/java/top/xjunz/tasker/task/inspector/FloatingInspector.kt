...
import top.xjunz.tasker.ui.task.inspector.NodeAdapter
...
class FloatingInspector {
    ...
    private val appContext = ContextThemeWrapper(app, R.style.Base)
    ...
    private fun showWindowNodeBounds() {
        ...
        if (windowRoot == null) {
            toast(R.string.inspect_failed)
        } else {
            inspector.setRootNode(StableNodeUtil.freeze(windowRoot))
        }
    }
